...
namespace otto::engines::nuke {
  ...
  struct Voice : voices::VoiceBase<Voice> {
    ...
    void set_frequencies(float lfo_val) noexcept;
    ...
    void on_state_change(const State& s) noexcept
    {
      ...
      env_filter_.release(s.envparam1_3);
  ...
  void Voice::set_frequencies(float lfo_val) noexcept
  {
    // TODO: the last factor can be precomputed for all voices
    float freq = frequency() * powf(2, 0.05 * lfo_val);
    osc1.freq(freq);
    osc2.freq(freq * osc2_freq_ratio);
    // filter_.setCutoff(1 + (15000 + 0.5 * frequency()) * cutoff * cutoff);
    filter_.setCutoff(
      1 +
      std::min((15000 + 0.5 * freq) * cutoff * cutoff * (1 + env_filter_amount * 5 * env_filter_()), 20000.0));
  }

  float Voice::operator()() noexcept
  {
    float lfo_val = lfo_.value();
    set_frequencies(lfo_val);
    ...
    return (after_filter + ring_mod) * env_() * (1) * 0.5f;
  }
